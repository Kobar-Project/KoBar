...
const usePluginStore = () => {
    const [state, setState] = useState({ cleanupMode: globalCleanupMode });
    useEffect(() => {
...
// --- Main Inline UI ---
const KoBoxWidget = () => {
    const [isKoBoxHovered, setIsKoBoxHovered] = useState(false);
    
    // Subscribe to cleanup logic
    useEffect(() => {
        // Trigger initial cleanup on mount just like the native App.tsx
...
const SettingsPanelUI = () => {
    ...
    return (
        <div className="flex flex-col gap-4 text-white">
            <h3 className="text-lg font-bold border-b border-white/10 pb-2">{t('kobox')} {t('settings')}</h3>
            
            <div className="flex flex-col gap-2">
                <label className="text-sm text-slate-300">{t('cleanupMode')}</label>
