    ...
    return (
        <div className="flex flex-col gap-4 text-slate-300">
            <div className="flex items-center justify-between">
                <span className="text-sm font-semibold">{t('maxCols')}</span>
                <input 
                    type="number" 
                    min="1" max="20" 
            ...
            <div className="flex items-center justify-between">
                <span className="text-sm font-semibold">{t('maxRows')}</span>
                <input 
                    type="number" 
                    min="1" max="20" 
...
const ShortcutsPanel = ({ onClose, anchorRect }) => {
    ...
    // 1. Independent State Management (localStorage)
    ...
    // 2. Global KoBar State
    const edgePosition = useAppStore(state => state.edgePosition);
    const orientation = useAppStore(state => state.orientation);
    const screenBounds = useAppStore(state => state.screenBounds) || { height: 800, width: 1200 };
                ...
                {pinnedApps.map((app, index) => {
                    ...
                    return (
                        <div
                            ...
                            onDragOver={(e) => e.preventDefault()}
                            onMouseDown={(e) => { if (e.shiftKey) setDragEnabled(true); }}
                            onMouseUp={() => setDragEnabled(false)}
                            className={`shortcut-card-plugin group relative animate-in fade-in transition-all ${
                        ...
                        >
                            {editingTagId === app.id ? (
                                ...
                                >
                                    ...
                                    />
                                    <span className="text-slate-500 mt-1 pointer-events-none" style={{ fontSize: `${Math.max(6, 7 * iconScale)}px` }}>{t('enterToSave')}</span>
                                </div>
                            ) : (
                                <div
                            ...
                            {editingTagId !== app.id && app.tag && (
                                <span className="absolute bottom-0 right-0 font-bold rounded-tl-lg rounded-br-xl uppercase tracking-wider pointer-events-none select-none z-20" style={{ backgroundColor: 'var(--theme-primary)', fontSize: `${Math.max(6, 8 * iconScale)}px`, padding: `${2 * iconScale}px ${4 * iconScale}px` }}>
                                    {app.tag}
                                </span>
                            )}
