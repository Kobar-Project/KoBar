...
const langs = ['en', 'tr', 'de', 'fr', 'es', 'ru', 'ar', 'zh', 'ja', 'hi'];

langs.forEach(lang => {
    const langStartIdx = text.indexOf(`    ${lang}: {`);
    if (langStartIdx !== -1) {
        let nextLangIdx = text.indexOf(`    },`, langStartIdx);
        if (lang === 'hi') {
            nextLangIdx = text.lastIndexOf(`    }`);
        }
        
        const langBlock = text.slice(langStartIdx, nextLangIdx);
        let additions = '';
        ...
        const newBlock = langBlock.replace(/(\\s*exportDataDesc:\\s*".*?",?)/, `$1${additions}`);
        text = text.slice(0, langStartIdx) + newBlock + text.slice(nextLangIdx);
    }
});
