...
import type { AnnotationTool } from '../../store/useScreenshotStore';
...
const AnnotationEditor: React.FC = () => {
    ...
    const getCompositeDataUrl = useCallback((): string => {
        ...
        // Get the original image dimensions for full-res export
        const dpr = window.devicePixelRatio || 1;
        return stage.toDataURL({
            pixelRatio: dpr,
            mimeType: exportFormat === 'jpg' ? 'image/jpeg' : exportFormat === 'webp' ? 'image/webp' : 'image/png',
            quality: exportFormat === 'png' ? 1 : 0.92,
        });
    }, [exportFormat]);
    ...
    const handleCopy = useCallback(() => {
        ...
        if (dataUrl) {
            window.api?.copyScreenshotToClipboard(dataUrl);
        }
    }, [getCompositeDataUrl]);
    ...
    const handleSave = useCallback(async () => {
        const dataUrl = getCompositeDataUrl();
        if (!dataUrl) return;
        await window.api?.saveScreenshot({
            buffer: dataUrl,
            format: exportFormat,
        });
    }, [getCompositeDataUrl, exportFormat]);

    const handleDone = useCallback(() => {
        resetScreenshot();
        window.api?.screenshotSessionComplete();
    }, [resetScreenshot]);
